...
# Prove the theorem u x (v x w) = (u • w)v - (u • v)w

# Let's introduce vector w
g, h, i = sp.symbols('w1, w2, w3')
w = sp.Matrix([g, h, i])

lh = sp.simplify(u.cross(v.cross(w)))
rh = sp.simplify((u.dot(w) * v) - (u.dot(v) * w))
print('u x (v x w) = (u • w)v - (u • v)w: ', lh.equals(rh))

# Prove the theorem (u x v) x w = (u • w)v - (v • w)u
